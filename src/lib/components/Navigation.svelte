...
<div>
  {#if innerWidth <= 1020}
    ...
    <div>
      <nav
        class="container relative px-6 py-5 mx-auto"
      >
        <div class="flex items-center justify-between">
            <button on:click={toggleNavbar} class="flex">
              <div
        ...
        >
            ...
            <li>
                {#if link.name !== "Logout"}
                  <a class="text-white" class:active="{$page.url.pathname === link.href}" href="{link.href}" on:click={toggleNavbar}>{link.name}</a>
                {/if}
            </li>
            {/each}
            {#if !$page.data.user}
            <li>
              <a href="/login" class="text-white" on:click={toggleNavbar}>Login</a>
            </li>
            {/if}
          ...
          {#if $page.data.user}
            <li>
              <form action="/logout" method="POST">
                <button type="submit" on:click={toggleNavbar}  class="text-white">Logout</button>
              </form>
            </li>  
          {/if}
        </ul>
      ...
      <div tabindex="2" class="overlay {showMenu ? 'nav-active' : 'nav-not-active'}" on:click={toggleNavbar} role="button" aria-roledescription="clickoverlay to exit menu"></div>
    </div>
    {:else}
    <!--Desktop-->
    <div>
      <nav
        class="container relative px-6 py-5 mx-auto flex justify-between items-center"
      >
        <div class="flex items-center justify-between">

          <a
            class="text-xl font-bold text-gray-800 text-2xl hover:text-gray-400"
            href="/"
            >
            <div class="block">
              <h2>
        ...
        >
            ...
            <li>
                {#if link.name !== "Logout"}
                  <a class="text-gray-800 hover:text-gray-400" class:active="{$page.url.pathname === link.href}" href="{link.href}">{link.name}</a>
                {/if}
            </li>
            {/each}
            {#if !$page.data.user}
          ...
          {#if $page.data.user}
            <li>
              <form action="/logout" method="POST">
                <button type="submit" class="text-gray-800 hover:text-gray-400">Logout</button>
              </form>
            </li>  
          {/if}
        </ul>
